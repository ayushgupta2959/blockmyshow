...
describe("BlockMyShow", () => {
  ...
  beforeEach(async () => {
    ...
    const BlockMyShow = await ethers.getContractFactory("BlockMyShow");
    blockMyShow = await BlockMyShow.deploy(NAME, SYMBOL);
  ...
  describe("Deployment", () => {
    it("Sets the name", async () => {
      expect(await blockMyShow.name()).to.equal(NAME);
    });

    it("Sets the symbol", async () => {
      expect(await blockMyShow.symbol()).to.equal(SYMBOL);
    });

    it("Sets the owner", async () => {
      expect(await blockMyShow.owner()).to.equal(deployer.address);
    });
  });
  ...
  describe("Withdrawing", () => {
    ...
    beforeEach(async () => {
      ...
      let transaction = await blockMyShow
        .connect(buyer)
        .mint(ID, SEAT, { value: AMOUNT });
      await transaction.wait();

      transaction = await blockMyShow.connect(deployer).withdraw();
      await transaction.wait();
    });
